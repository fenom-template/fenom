...
* `$.const` обращение к PHP константе:
  * `$.const.PHP_EOL` — возвращает значение константы `PHP_EOL`
  * Пространства имен через точку: `$.const.Storage.FS::DIR_SEPARATOR` возвращает `Storage\FS::DIR_SEPARATOR`
  * Массивы-константы: доступ к элементам через точку: `$.const.GEO.id` где `GEO` определена как `define('GEO', ['id' => 123])`
